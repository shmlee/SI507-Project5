...
def get_tokens(client_key=CLIENT_KEY, client_secret=CLIENT_SECRET,request_token_url=REQUEST_TOKEN_URL,base_authorization_url=BASE_AUTH_URL,access_token_url=ACCESS_TOKEN_URL,verifier_auto=False):
    ...
    auth_url = oauth_inst.authorization_url(base_authorization_url) #will give the url to get final approval for verifiers
    ...
    if verifier_auto: # if the input is default (True), like Twitter, if there's a verifiers
    ...
    # Regenerate instance of oauth1session class with more data, meanging get new instances
...
if __name__ == "__main__":
    ...
    # Invoke functions
    tumblr_search_baseurl = "https://api.tumblr.com/v2/blog/isharonlee.tumblr.com/info"
    tumblr_api_key= CLIENT_SECRET
    tumblr_blog_identifier = "isharonlee.tumblr.com"
    tumblr_info_params = {"blog_identifier": "tumblr_blog_identifier", "api_key":"tumblr_blog_identifier"}
    # tumblr_search_params = {'q':
    # "University of Michigan", "count":4}
    tumblr_result = get_data_from_api(tumblr_search_baseurl,"tumblr",tumblr_info_params) # Default expire_in_days
    # print(type(tumblr_result))


    tumblr_search_baseurl2 = "https://api.tumblr.com/v2/blog/isharonlee.tumblr.com/following"
    tumblr_follower_params = {"limit":5}
    tumblr_result2 = get_data_from_api(tumblr_search_baseurl2,"tumblr",tumblr_follower_params)
    tumblr_list = tumblr_result2["response"]["blogs"]
    tumblr_rows = [(x["name"],x["title"],x["url"]) for x in tumblr_list]
    print (tumblr_rows)
    # print(tumblr_result2)


    tumblr_search_baseurl3 = "https://api.tumblr.com/v2/tagged"
    tag = "chocolate"
    tumblr_tags_params = {"tag":"chocolate","limit":5}
    tumblr_result3 = get_data_from_api(tumblr_search_baseurl3,"tumblr",tumblr_tags_params)
    tumblr_list2 = tumblr_result3["response"]
    tumblr_rows2 = [(y["type"],y["blog_name"],y["tags"]) for y in tumblr_list2]
    print(tumblr_rows2)
    # print (json.dumps(tumblr_result3, indent=2))
    # print(tumblr_result3)
...
with open('followers.csv', 'w', newline='') as f:
    ...
    for obj in tumblr_rows:
        writer.writerow(obj)

with open("chocolate.csv", 'w', newline='') as f:
    writer = csv.writer(f)
    f.write("Type, Blog Name, Tags\n")
    for obj in tumblr_rows2:
        writer.writerow([obj[0], obj[1], ', '.join(obj[2])])
